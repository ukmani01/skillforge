#!/usr/bin/env node
// D:\todo-application\backend\backendfullcode-version2.js
// CLI generator for the SkillForge backend.
// Run from backend folder:  node backendfullcode-version2.js
...
console.log('');
console.log('=================================================');
console.log(' SkillForge — Backend generator (v2)');
...
// Reserved for future use — questions live in backend/data/*.json.
...
module.exports = async function studentAuth(req, res, next) {
  try {
    ...
    const student = await Student.findById(req.session.studentId).select('-passwordHash');
    if (!student) {
...
module.exports = { isNonEmptyString: isNonEmptyString, isValidEmail: isValidEmail };
`);
...
router.post('/attempts', async function (req, res) {
  try {
    ...
    console.error('POST /attempts:', error.message);
...
router.delete('/attempts/student/:email', async function (req, res) {
  try {
    ...
    res.json({ success: true, message: 'Deleted ' + result.deletedCount + ' attempts.' });
  } catch (error) {
...
router.get('/leaderboard', async function (req, res) {
  try {
    ...
    const filter = {};
    if (year && year !== 'all') filter.year = year;
    const attempts = await Attempt.find(filter).sort({ percentage: -1, score: -1, createdAt: 1 }).lean();

    const best = {};
    attempts.forEach(function (a) {
      const k = a.studentEmail;
      if (!best[k] || a.percentage > best[k].percentage) best[k] = a;
    });

    const leaderboard = Object.values(best)
      .sort(function (a, b) { return (b.percentage - a.percentage) || (b.score - a.score); })
      .map(function (a, i) {
        return {
          rank: i + 1, _id: a._id, studentName: a.studentName, studentEmail: a.studentEmail,
          year: a.year, score: a.score, totalQuestions: a.totalQuestions,
          percentage: a.percentage, level: a.level, attemptNumber: a.attemptNumber, createdAt: a.createdAt
        };
      });
...
router.get('/dashboard/stats', async function (req, res) {
  try {
    ...
    const stats = {
      totalStudents: 0, totalAttempts: all.length,
      averageScore: 0, averagePercentage: 0, highestScore: 0, lowestScore: 40,
      yearStats: { '2nd': { count: 0, avgPercent: 0 }, '3rd': { count: 0, avgPercent: 0 } },
      levelDistribution: {}, easyAccuracy: 0, mediumAccuracy: 0, hardAccuracy: 0, interviewAccuracy: 0
    };
    ...
    all.forEach(function (a) {
      eC += a.easyScore || 0; mC += a.mediumScore || 0;
      hC += a.hardScore || 0; iC += a.interviewScore || 0;
    });
...
router.post('/admin/login', function (req, res) {
  ...
  if (body.email === config.adminEmail && body.password === config.adminPassword) {
    ...
    req.session.adminEmail = body.email;
...
router.get('/students/me', studentAuth, function (req, res) {
  res.json({
    success: true,
    student: {
      _id: req.student._id, name: req.student.name, email: req.student.email,
      year: req.student.year, createdAt: req.student.createdAt
    }
  });
});
...
router.get('/students/me/attempts', studentAuth, async function (req, res) {
  try {
    const attempts = await Attempt.find({ studentEmail: req.student.email }).sort({ createdAt: -1 }).lean();
...
router.post('/feedback', studentAuth, async function (req, res) {
  try {
    ...
    const comment = b.comment ? String(b.comment).trim().slice(0, 1000) : '';
...
router.get('/feedback/mine', studentAuth, async function (req, res) {
  try {
    const items = await Feedback.find({ studentId: req.student._id }).sort({ createdAt: -1 }).lean();
...
router.get('/posts/mine', studentAuth, async function (req, res) {
  try {
    const posts = await Post.find({ studentId: req.student._id }).sort({ createdAt: -1 }).lean();
    res.json({ success: true, posts: posts });
  } catch (error) {
    res.status(500).json({ success: false, error: 'Failed to fetch posts.' });
  }
...
function loadQuestions(year) {
  const file = path.join(DATA_DIR, year === '2nd' ? 'second-year.json' : 'third-year.json');
...
// GET /api/quiz/questions?year=2nd&level=1
// Returns questions WITHOUT correct answer (safe for client).
router.get('/quiz/questions', function (req, res) {
  const year = req.query.year === '3rd' ? '3rd' : '2nd';
  const level = Number(req.query.level) === 2 ? 2 : 1;

  const data = loadQuestions(year);
  if (!data || !Array.isArray(data.questions)) {
    return res.status(500).json({ success: false, error: 'Question data unavailable.' });
  }

  const set = data.questions.filter(function (q) { return q.level === level; });
  const safe = set.map(function (q) {
    return {
      id: q.id,
      question: q.question,
      options: q.options,
...
// Server validates answers and returns computed score. Does NOT save attempt.
router.post('/quiz/submit', function (req, res) {
  ...
  const set = data.questions.filter(function (q) { return q.level === level; });
  if (set.length === 0) {
    return res.status(404).json({ success: false, error: 'No questions for this level.' });
  }

  let correct = 0;
  const results = set.map(function (q) {
    const a = answers.find(function (x) { return x && String(x.questionId) === String(q.id); });
    const selected = a ? Number(a.selectedOption) : null;
...
// ============================================================
// data/second-year.json  (40 placeholder slots ready to fill)
// ============================================================
write('data/second-year.json', JSON.stringify({
  version: '1.0',
  year: '2nd',
  subject: 'HTML Fundamentals',
  note: 'Fill these 40 questions by editing this file. Level 1 = first 20, Level 2 = next 20.',
  questions: []
}, null, 2) + '\n');
...
module.exports = function attachSocket(httpServer) {
  ...
  io.on('connection', function (socket) {
    ...
    socket.on('join', function (room) {
      const safe = String(room || 'general').slice(0, 100);
      socket.join(safe);
    });
...
db.connect().then(function () {
  ...
  server.listen(config.port, function () {
    console.log('SkillForge on port ' + config.port);
...
process.on('uncaughtException', function (err) { console.error('Uncaught:', err); process.exit(1); });
`);

console.log('');
console.log('=================================================');
console.log(' Backend generator v2 complete.');
...
console.log('  3. copy .env.example .env   (then edit)');
console.log('  4. npm start');
console.log('');
console.log('Then run the frontend generator.');
console.log('');